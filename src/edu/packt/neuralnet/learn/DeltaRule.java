...
public class DeltaRule extends LearningAlgorithm {
    ...
    public Double calcNewWeight(int layer, int input, int neuron)
        ...
        if (layer > 0) {
            throw new NeuralException("Дельта-правило может быть использовано только с однослойной"
                    + " нейронной сетью");
        } else {
    ...
    public void train() throws NeuralException {
        if (neuralNet.getNumberOfHiddenLayers() > 0) {
            throw new NeuralException("Дельта-правило может быть использовано только с однослойной нейронной сетью");
        } else {
            switch (learningMode) {
                case BATCH:
                    epoch = 0;
    ...
    public void print() {
        if (learningMode == LearningMode.ONLINE)
            System.out.println("Эпоха=" + String.valueOf(epoch) + "; Запись="
                    + String.valueOf(currentRecord) + "; Общая ошибка="
                    + String.valueOf(overallGeneralError));
        else
            System.out.println("Эпоха= " + String.valueOf(epoch)
                    + "; Общая ошибка =" + String.valueOf(overallGeneralError));
    }
